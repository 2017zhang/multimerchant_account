    ...
    <div class="classify">
        <div class="g_att">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="Commodity-management">
                <tr>
                    ...
                    <td class="inp">
                        ...
                        <div class="select" >
                            <p class="select-title" @click="clickSize()">{{timeSlotContent}}<i class="el-icon-arrow-down el-icon--right"></i></p> 
                            <transition name="opacityLeave">
                                <div class="condition-list" v-show="isShowSize">
                                    <div class="size-wrap">
                                        <p :class="colorHover==index?'active':'a_white'" class="size" @mouseenter="changeColor(index)" @mouseleave="reductionColor(index)"  :key="index" v-for="(item,index) in ringSize" @click="getSize (item) ">{{formatDate(item.start_time)}}-{{formatDate(item.end_time)}}</p>
                                    </div>
                                </div>
                            </transition>
                        </div>
                    ...
                    <td class="inp">
                        <el-input style="width: 230px;" v-model="params.activeName" clearable></el-input>
                    </td>
                    <td align="left">
                        <el-button @click="createDistribution()" type="success">生成分销</el-button>
                    </td>
                </tr>
            </table>
            <el-table border :data="dataTable" style="width: 100%;text-align: center;" max-width="900">
                ...
                <el-table-column width="200" show-overflow-tooltip prop="title" label="标题">
                </el-table-column>
                ...
                <el-table-column width="140" label="操作">
                    <template slot-scope="scope">
                        ...
                        <el-button @click="toDetail(scope.row.id)" type="success">分销详情</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <div class="Paging">
    ...
    export default {
        ...
        data() {
            return {
                ...
                ringSize:[{"id":1,"AttrValueName":"2018年10月11日-2019年11月20日"},{"id":2,"AttrValueName":"2018年11月12日-2019年12月20日"}]
            }
        },
        created() {
        },
        ...
        methods: {
            ...
            createDistribution(){ // 生成分销
                if(this.times == ""){
                    this.$message.error("请选择时间段")
                    return
                }
                this.$HTTP(this.$httpConfig.memberDistribution,{times:this.times,title:this.params.activeName}).then((res)=>{
            ...
            // 获取下拉时间段
            getTimeSolt(){
                this.$HTTP(this.$httpConfig.distributionPeriod,{}).then((res) => {
            ...
            getSize (item) {
                ...
                this.timeSlotContent = this.formatDate(item.start_time)+"-"+this.formatDate(item.end_time);
            ...
            getData() {
                ...
                this.$HTTP(this.$httpConfig.getFullGiftList,{page:this.params.page}).then((res) => {
