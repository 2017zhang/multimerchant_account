        ...
        <div class="g_att">
            ...
            <div class="notes-box">
                ...
                <div class="notes_content">
                    <p>是否主客服：只能设置其中一个客服为主客服，若想切换客服，必须先把原来的客服设置为不是主客服</p>
                </div>
            </div>
            <el-table border :data="dataTable" style="width: 100%" max-width="900">
                ...
                <el-table-column fixed="right" width="200" label="操作">
                    <template slot-scope="scope">
                        <el-button @click="editItem(scope.row.id)" size="mini" icon="el-icon-edit"></el-button>
...
export default {
    ...
    methods: {
        ...
        getData() {
            this.$HTTP(this.$httpConfig.getServiceManageList,{}).then((res) => {
        ...
        setMain(id, status, index) {
            this.$HTTP(this.$httpConfig.setServiceMain,{
                ...
                this.$message.error(res.data.message + '，请先将其他主客服关闭！');
            })
        },
        editItem(value) {
            this.$router.push({
        ...
        deleteItem(value) {
            this.$confirm('此操作将永久删除此客服，是否继续？', '提示', {
                ...
                this.$HTTP(this.$httpConfig.delService,{service_id: value}).then((res) => {
                    this.$message.success(res.data.message);
                    this.getData();
                }).catch((err) => {
                    this.$message.error(err);
